...
// T-table optimized SM4 round
void sm4_ttable_round(uint32_t *state, uint32_t rk) {
    uint32_t x = state[1] ^ state[2] ^ state[3] ^ rk;
    uint8_t *b = (uint8_t*)&x;
    uint32_t t = T_table[b[0]] ^ T_table[b[1]] ^ T_table[b[2]] ^ T_table[b[3]];
    state[0] ^= t;
    // Rotate state
    uint32_t tmp = state[0];
    state[0] = state[1];
...
void init_ttable(void) {
    for (int i = 0; i < 256; i++) {
        uint32_t t = ((uint32_t)Sbox[i]) << 24;
        T_table[i] = t ^ ((t << 2) | (t >> 30)) ^ ((t << 10) | (t >> 22)) ^
                     ((t << 18) | (t >> 14)) ^ ((t << 24) | (t >> 8));
    }
}
